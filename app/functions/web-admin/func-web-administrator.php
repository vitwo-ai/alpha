...
function administratorAuth()
{
    ...
    if (!isset($_SESSION["logedWebAdminInfo"]["adminId"]) || !isset($_SESSION["logedWebAdminInfo"]["adminRole"])) {
        redirect(WEB_ADMIN_URL . "login.php");
    } else {
        $adminRole = $_SESSION["logedWebAdminInfo"]["adminRole"];
        if ($adminRole != 1) {
            $currentPage = basename($_SERVER['PHP_SELF']);
            if (!in_array($currentPage, $commonAccessPages)) {
                $sqlAccesses = "SELECT `fldRoleAccesses` FROM `tbl_web_admin_roles` WHERE `fldRoleStatus`='active' AND `fldRoleKey`=" . $adminRole;
                $accesses = "";
                if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
                    $accesses = mysqli_fetch_assoc($qryAccesses)["fldRoleAccesses"];
                } else {
                    redirect(WEB_ADMIN_URL);
                }
                $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_web_admin_menu` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldMenuFile`='".$currentPage."'";
...
function getAdministratorMenuSubMenu()
{
    ...
    $adminRole = $_SESSION["logedWebAdminInfo"]["adminRole"];
    if($adminRole==1){
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_web_admin_menu` WHERE `fldParentMenuKey`!=0 AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
    ...
    $parentMenuKeysArr=[];
    $sqlParentMenuKeys = "SELECT `fldParentMenuKey` FROM `tbl_web_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' GROUP BY `fldParentMenuKey`;";
    ...
    $sqlMenuList="SELECT * FROM `tbl_web_admin_menu` WHERE `fldMenuKey` IN (".$parentMenuKeys.") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
    if($qryMenuList=mysqli_query($dbCon,$sqlMenuList)){
        if (mysqli_num_rows($qryMenuList)>0) {
            ...
            while($rowMenuList=mysqli_fetch_assoc($qryMenuList)){
                ...
                $sqlSubMenuList="SELECT * FROM `tbl_web_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=".$parentMenuKey." ORDER BY `fldMenuOrderBy` ASC";
...
function loginAdministratorUser($POST){
    global $dbCon;
    $returnData=[];
    $sql="SELECT * FROM `tbl_web_admin_details` WHERE `fldAdminEmail`='".$POST["email"]."' AND `fldAdminStatus`='active'";
    if($result=mysqli_query($dbCon,$sql)){
        if(mysqli_num_rows($result)>0){
            $row=mysqli_fetch_assoc($result);
            if($POST["pass"]==$row["fldAdminPassword"]){
                $_SESSION["logedWebAdminInfo"]["adminId"]=$row["fldAdminKey"];
                $_SESSION["logedWebAdminInfo"]["adminName"]=$row["fldAdminName"];
                $_SESSION["logedWebAdminInfo"]["adminEmail"]=$row["fldAdminEmail"];
                $_SESSION["logedWebAdminInfo"]["adminPhone"]=$row["fldAdminPhone"];
                $_SESSION["logedWebAdminInfo"]["adminRole"]=$row["fldAdminRole"];
                $returnData["status"]="success";
...
function saveAdministratorSettings($POST)
{
    ...
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["title"]."' WHERE `fldSettingName`='title'");
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["timeZone"]."' WHERE `fldSettingName`='timeZone'");
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["email"]."' WHERE `fldSettingName`='email'");
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["phone"]."' WHERE `fldSettingName`='phone'");
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["address"]."' WHERE `fldSettingName`='address'");
    array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$POST["footer"]."' WHERE `fldSettingName`='footer'");
    ...
    if($logoObj["status"]=="success"){
        ...
        array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$logoObj["data"]."' WHERE `fldSettingName`='logo'");
    }
    if($faviconObj["status"]=="success"){
        ...
        array_push($sql, "UPDATE `tbl_web_admin_settings` SET `fldSettingValue`='".$faviconObj["data"]."' WHERE `fldSettingName`='favicon'");
    }
...
function addAdministratorMenu($POST)
{
    ...
    $ins = "INSERT INTO `tbl_web_admin_menu`
                SET
...
function updateAdministratorMenu($POST)
{
    ...
    if ($res = mysqli_query($dbCon, "SELECT COUNT(`fldMenuLabel`) AS 'noOfRecords' FROM `tbl_web_admin_menu` WHERE `fldMenuKey`=" . $POST["menuKey"])) {
        if (mysqli_fetch_assoc($res)["noOfRecords"] == 1) {
            $ins = "UPDATE `tbl_web_admin_menu`
                SET
                    `fldMenuLabel` = '" . $menuLabel . "',
                    `fldMenuIcon` = '" . $menuIcon . "',
                    `fldMenuFile` = '" . $menuFile . "',
...
function getAdministratorMenuList($parentMenuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_web_admin_menu` WHERE `fldParentMenuKey`=" . $parentMenuKey . " AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
...
function getAdministratorMenuDetails($menuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_web_admin_menu` WHERE `fldMenuKey`=" . $menuKey;
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function addNewAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_web_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_web_admin_roles` SET
...
function updateAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_web_admin_roles` WHERE `fldRoleKey`=" . $roleKey . " AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 1) {

                $checkName = "SELECT * FROM `tbl_web_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleKey`!=" . $roleKey . " AND  `fldRoleStatus`!='deleted'";
                ...
                $ins = "UPDATE `tbl_web_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
...
function getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_web_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleKey`=" . $key . "";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorRoles()
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_web_admin_roles` WHERE `fldRoleStatus`!='deleted'";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorUsers()
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_web_admin_details`,`tbl_web_admin_roles` WHERE `tbl_web_admin_details`.`fldAdminRole`=`tbl_web_admin_roles`.`fldRoleKey` AND `tbl_web_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_web_admin_roles`.`fldRoleStatus`!='deleted'";
...
function getAdministratorUserDetails($key = null)
{
    ...
    $sql = "SELECT `tbl_web_admin_details`.*, `tbl_web_admin_roles`.`fldRoleName` FROM `tbl_web_admin_details`,`tbl_web_admin_roles` WHERE `tbl_web_admin_details`.`fldAdminRole`=`tbl_web_admin_roles`.`fldRoleKey` AND `tbl_web_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_web_admin_roles`.`fldRoleStatus`!='deleted' AND `fldAdminKey`=" . $key;
...
function updateAdministratorUserDetails($POST)
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_web_admin_details` WHERE `fldAdminKey`='" . $adminKey . "'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $ins = "UPDATE `tbl_web_admin_details` 
                            SET
...
function addNewAdministratorUser($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_web_admin_details` WHERE `fldAdminEmail`='" . $adminEmail . "' AND `fldAdminStatus`!='deleted'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_web_admin_details` 
                            SET
                                `fldAdminName`='" . $adminName . "',
                                `fldAdminEmail`='" . $adminEmail . "',
                                `fldAdminPhone`='" . $adminPhone . "',
...
function getAdministratorAccessesNames($roleAccesses = "0")
{
    ...
    if ($roleAccesses == "0") {
        ...
        $sql = "SELECT `fldMenuLabel` FROM `tbl_web_admin_menu` WHERE `fldMenuKey` IN (" . $roleAccesses . ") AND `fldMenuStatus`='active'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $returnData["status"] = "success";
                $returnData["message"] = "success";
